        ...
        success: function (data) {
            ...
            if (data.data.results.length > 0) {
                ...
                data.data.results.forEach(photo => {
                    ...
                    strToRender += makeImageContainer(photo.urls.small, description, photo.id);
                });
...
function addPhoto(photo_unsplash_id, collection_id, path) {
    const url = `/photo/add`;
    $.ajax({
        url,
        type: "GET",
...
function makeImageContainer(url, description, photoId) {
    if($.isArray(collections)){
        strCollectionsOp = collections.map(
            collection => `<button class="dropdown-item"
        onclick="addPhoto('${photoId}', ${collection.id}, '${url}')">${collection.name}</button>`
        ).join("");
        ...
        onclick="addPhoto('${photoId}', ${collections}, '${url}')"> Add to collection</button>`+
            `</div>`+
            `</div>`;
    }
